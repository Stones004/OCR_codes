...
class AnnotationDetector:
    ...
    def __init__(
        ...
        # ---- new tuning knobs ----
        min_area=50,
        merge_dist_extended=40,
    ):
        self.min_width = min_width
        self.min_height = min_height
        self.padding = padding

        # STAGE 1 — per-fragment noise floor, applied BEFORE merging.
        # Kept tight/low: this only needs to kill lone speckle, not real
        # character fragments (a fragmented "2" or ")" must survive here).
        self.min_component_area = min_component_area

        # STAGE 2 — join distance used AFTER stage-1 noise is already gone.
        # Safe to be generous here since noise can't get re-introduced —
        # this is the knob that joins split characters like "2" + ")".
        self.merge_dist = merge_dist

        # STAGE 3 — final confidence check on the fully merged blob.
        self.min_confident_area = min_confident_area
        self.min_confident_dim = min_confident_dim
        ...
        self.merge_dist_extended = merge_dist_extended
    ...
    def _boxes_close(self, a, b, dist):
        ax1, ay1, ax2, ay2 = a
        bx1, by1, bx2, by2 = b
        ax1 -= dist; ay1 -= dist
        ax2 += dist; ay2 += dist
    ...
    def remove_small_components(self, binary, min_area=200):
    ...
    def _merge_boxes(self, boxes, dist):
        # boxes: list of (x1,y1,x2,y2). Repeated pairwise merge until stable.
        changed = True
        while changed:
            changed = False
            ...
            for i in range(len(boxes)):
                ...
                for j in range(i + 1, len(boxes)):
                    if used[j]:
                        continue
                    if self._boxes_close(tuple(cur), boxes[j], dist):
                        cur[0] = min(cur[0], boxes[j][0])
                        cur[1] = min(cur[1], boxes[j][1])
                        cur[2] = max(cur[2], boxes[j][2])
                        cur[3] = max(cur[3], boxes[j][3])
    ...
    def detect(self, image):
        ...
        # ---- STAGE 1: kill noise per-component, BEFORE merging ----
        # A lone speck's own area/ink is what makes it noise, not its
        # distance to neighbours — decide this independent of merge_dist.
        survivors = []
        for i in range(1, num_labels):
            w = stats[i, cv2.CC_STAT_WIDTH]
            h = stats[i, cv2.CC_STAT_HEIGHT]
            ...
            survivors.append((x, y, x + w, y + h))

        # ---- STAGE 2: merge survivors with a GENEROUS distance ----
        # Noise is already gone, so it's now safe to join fragments of
        # the same word/number (e.g. "2" + ")") without dragging
        # speckle back in through over-eager merging.
        merged = self._merge_boxes(survivors, self.merge_dist)

        # ---- STAGE 3: final confidence check on the merged blob ----
        confident = []
        for (x1, y1, x2, y2) in merged:
            w = x2 - x1
            h = y2 - y1
